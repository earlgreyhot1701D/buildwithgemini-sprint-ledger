...
def _extract_a2ui_messages(text: str) -> list[dict]:
    ...
    for value in _iter_json_values(text):
        if isinstance(value, list):
            # Check if this list is a list of components: [{"id": ..., "component": ...}, ...]
            if value and all(isinstance(x, dict) and "component" in x for x in value):
                root_id = value[0].get("id", "root")
                return [
                    {"beginRendering": {"surfaceId": "default", "root": root_id}},
                    {"surfaceUpdate": {"surfaceId": "default", "components": value}},
                ]
            values.extend(value)
        else:
            values.append(value)
    ...
    for value in values:
        ...
        elif "components" in value and isinstance(value["components"], list):
            # Model emitted shorthand single-object surface {surfaceId, components: [...]}
            surface_id = value.get("surfaceId", "default")
            root_id = value["components"][0].get("id", "root") if value["components"] else "root"
            messages.append({"beginRendering": {"surfaceId": surface_id, "root": root_id}})
            messages.append({"surfaceUpdate": {"surfaceId": surface_id, "components": value["components"]}})
    return messages
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if not any(k in text for k in _A2UI_KEYS) and "a2ui-json" not in text and '"component"' not in text:
            continue
